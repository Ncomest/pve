...
<template>
  <section class="inventory-section">
    ...
    <div class="inventory-section__body">
      ...
      />
      <div
        v-if="props.selectedDisplayItem"
        class="inventory-section__item-details"
      >
        <div class="inventory-section__comparison-title">В инвентаре:</div>
        <ItemDetails
      ...
      <div
        v-if="props.showEquippedInInventory && props.selectedDisplayItem"
        class="inventory-section__comparison"
      >
        <div class="inventory-section__comparison-title">Надето сейчас:</div>
        <ItemDetails
          v-if="props.selectedEquippedDisplayItem"
          :item="props.selectedEquippedDisplayItem"
          :is-equipped="true"
          :is-equipped-slot="true"
          @unequip="emit('unequipSelected')"
        />
        <div v-else class="inventory-section__comparison-empty">
          В этом слоте сейчас ничего не надето.
        </div>
      </div>
    </div>
  </section>
  ...
  <Teleport to="body">
    <div
      v-if="props.inventoryFullWarning"
      class="inventory-warning-overlay"
      @click.self="emit('closeWarning')"
    >
      <div class="inventory-warning-modal">
        ...
        <p class="inventory-warning-modal__text">
          Нет свободных слотов для снятия предмета. Освободите место в
          инвентаре.
        </p>
        <button
          type="button"
