<script setup lang="ts">
  ...
  const MANUAL_REFRESH_COST_GOLD = 50 * (playerProgress.state.level ?? 1);
  ...
  function selectSlot(
    _item: import("@/entities/item/model").ItemInstance | null,
    index: number,
  ) {
  ...
  function selectConsumableSlot(
    _item: import("@/entities/item/model").ItemInstance | null,
    index: number,
  ) {
    selectedConsumableIndex.value =
      selectedConsumableIndex.value === index ? null : index;
    selectedIndex.value = null;
  }
  ...
  function sellSelectedElixir() {
    if (selectedConsumableIndex.value === null || !selectedConsumableItem.value)
      return;
    const earned = characterStore.sellItemFromConsumables(
      selectedConsumableIndex.value,
    );
    if (earned > 0) {
      showMessage(`Продано за ${earned} золотых.`);
  ...
  function getEquipmentOfferTooltip(offer: MerchantOffer): string {
    ...
    if (item.stats.chanceCrit != null)
      lines.push(`Крит: +${item.stats.chanceCrit}`);
    if (item.stats.evasion != null)
      lines.push(`Уклонение: +${item.stats.evasion}`);
    if (item.stats.speed != null) lines.push(`Скорость: +${item.stats.speed}`);
    if (item.stats.armor != null) lines.push(`Броня: +${item.stats.armor}`);
    if (item.stats.accuracy != null)
      lines.push(`Меткость: +${item.stats.accuracy}`);
    if (item.stats.critDefense != null)
      lines.push(`Защита от крита: +${item.stats.critDefense}`);
    if (item.stats.lifesteal != null)
      lines.push(`Самоисцеление: +${item.stats.lifesteal}`);
  ...
  function getElixirOfferTooltip(elixir: (typeof ELIXIRS)[number]): string {
    ...
    switch (def.kind) {
      ...
      case "armor_percent":
        lines.push(
          `Броня +${Math.round((def.armorPercentBonus ?? 0) * 100)}%.`,
        );
        break;
      case "crit_percent":
      ...
      case "speed_percent":
        lines.push(
          `Скорость +${Math.round((def.speedPercentBonus ?? 0) * 100)}%.`,
        );
        break;
      case "health_percent":
      ...
      case "evasion_percent":
        lines.push(
          `Уклонение +${Math.round((def.evasionPercentBonus ?? 0) * 100)}%.`,
        );
        break;
      default:
        break;
  ...
  function getElixirOfferDescriptionLines(
    elixir: (typeof ELIXIRS)[number],
  ): string[] {
  ...
  function toggleEquipmentOffer(offerId: string) {
    expandedEquipmentOfferId.value =
      expandedEquipmentOfferId.value === offerId ? null : offerId;
  }

  function toggleElixirOffer(elixirId: string) {
    expandedElixirId.value =
      expandedElixirId.value === elixirId ? null : elixirId;
  }
  ...
  function buyItem(offer: MerchantOffer) {
    ...
    if (ok) {
      ...
      saveOffersToStorage();
      showMessage(
        `${display?.name ?? "Предмет"} куплен за ${offer.price} золотых.`,
      );
    } else {
  ...
  function loadOrRegenerateOffers() {
    ...
    try {
      ...
      const storedHeroLevel =
        typeof parsed.heroLevelAtGeneration === "number"
          ? parsed.heroLevelAtGeneration
          : -1;
      const storedNextRefreshAt =
        typeof parsed.nextRefreshAt === "number" ? parsed.nextRefreshAt : 0;
      const storedOffers = Array.isArray(parsed.offers)
        ? (parsed.offers as MerchantOffer[])
        : [];
    ...
    <div class="merchant-page__content">
      <section class="merchant-page__section merchant-page__section--inventory">
        ...
        <div class="merchant-page__inventory-body">
          <div
            v-if="selectedDisplayItem"
            class="merchant-page__sell-card merchant-page__inventory-panel--sell-equipment"
          >
            <h3
            ...
            <div
              v-if="selectedDisplayItem.itemLevel != null"
              class="merchant-page__item-level"
            >
              Ур. вещи: {{ selectedDisplayItem.itemLevel }}
            </div>
            <div class="merchant-page__item-stats">
              <div
                v-if="selectedDisplayItem.stats.hp != null"
                class="merchant-page__item-stat"
              >
                HP: +{{ selectedDisplayItem.stats.hp }}
              </div>
              <div
                v-if="selectedDisplayItem.stats.power != null"
                class="merchant-page__item-stat"
              >
                Атака: +{{ selectedDisplayItem.stats.power }}
              </div>
              <div
                v-if="selectedDisplayItem.stats.chanceCrit != null"
                class="merchant-page__item-stat"
              >
                Крит: +{{ selectedDisplayItem.stats.chanceCrit }}
              </div>
              <div
                v-if="selectedDisplayItem.stats.evasion != null"
                class="merchant-page__item-stat"
              >
                Уклонение: +{{ selectedDisplayItem.stats.evasion }}
              </div>
              <div
                v-if="selectedDisplayItem.stats.speed != null"
                class="merchant-page__item-stat"
              >
                Скорость: +{{ selectedDisplayItem.stats.speed }}
              </div>
              <div
                v-if="selectedDisplayItem.stats.armor != null"
                class="merchant-page__item-stat"
              >
                Броня: +{{ selectedDisplayItem.stats.armor }}
              </div>
              <div
                v-if="selectedDisplayItem.stats.accuracy != null"
                class="merchant-page__item-stat"
              >
                Меткость: +{{ selectedDisplayItem.stats.accuracy }}
              </div>
              <div
                v-if="selectedDisplayItem.stats.critDefense != null"
                class="merchant-page__item-stat"
              >
                Защита от крита: +{{ selectedDisplayItem.stats.critDefense }}
              </div>
              <div
                v-if="selectedDisplayItem.stats.lifesteal != null"
                class="merchant-page__item-stat"
              >
          ...
          <div
            v-if="selectedElixirDef"
            class="merchant-page__sell-card merchant-page__inventory-panel--sell-elixir"
          >
            <h3 class="merchant-page__item-name">
              {{ selectedElixirDef.name }}
            </h3>
            <div class="merchant-page__elixir-description">
              {{ selectedElixirDescription }}
            </div>
            <div class="merchant-page__sell-price">
              <img
                src="/images/currencies/coin.png"
      ...
      <section class="merchant-page__section merchant-page__section--stock">
        <div class="merchant-page__tabs">
          <button
            ...
            :class="{
              'merchant-page__tab-btn--active': activeTab === 'equipment',
            }"
          ...
          <button
            ...
            class="merchant-page__tab-btn"
            :class="{
              'merchant-page__tab-btn--active': activeTab === 'consumables',
            }"
        ...
        <template v-if="activeTab === 'equipment'">
          ...
          <div class="merchant-page__stock-actions">
            ...
            <div v-if="nextRefreshLabel" class="merchant-page__refresh-label">
              {{ nextRefreshLabel }}
            </div>
          </div>
          <div class="merchant-page__stock-list">
            <div
              ...
              :class="{
                'merchant-page__offer--expanded':
                  expandedEquipmentOfferId === row.offer.id,
              }"
            >
              <template v-if="row.displayItem">
                ...
                <div class="merchant-page__offer-info">
                  ...
                  <span class="merchant-page__offer-level"
                    >Ур. {{ row.displayItem.itemLevel ?? 1 }}</span
                  >
        ...
        <template v-else>
          ...
          <div class="merchant-page__stock-list">
            <div
              ...
              :class="{
                'merchant-page__offer--expanded':
                  expandedElixirId === elixir.id,
              }"
            >
              <div
                class="merchant-page__offer-icon merchant-page__offer-icon--elixir"
              >
                <img
                  :src="elixir.icon"
                  :alt="elixir.name"
